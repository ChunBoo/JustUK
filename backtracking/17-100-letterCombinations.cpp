...
public:
    std::vector<string> letterCombinations(string digits) {
        if(digits.empty())
            return {};
        std::vector<std::vector<char>> d(10);
        d[0]={' '};
        d[1]={};
        d[2]={'a','b','c'};
        d[3]={'d','e','f'};
        ...
        string cur;
        dfs(digits,cur,0,ans,d);
        return ans;
    }
private:
    void dfs(const string& digits,string& cur,int l,std::vector<string>& ans,const std::vector<std::vector<char>>& d){
...
int main()
{
    vector ret=Solution3().letterCombinations("23");
    for(auto s:ret)
        cout<<s<<',';
}